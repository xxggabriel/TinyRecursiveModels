...
def train_batch(config: PretrainConfig, train_state: TrainState, batch: Any, global_batch_size: int, rank: int, world_size: int):
    ...
    if len(metrics):
        ...
        if rank == 0:
            metric_values = metric_values.cpu()
            reduced_metrics = {k: metric_values[i].item() for i, k in enumerate(metric_keys)}

            count = max(reduced_metrics.get("count", 0.0), 1.0)
            accuracy_sum = reduced_metrics.pop("accuracy", 0.0)
            accuracy_sq_sum = reduced_metrics.pop("accuracy_sq", 0.0)
            f1_sum = reduced_metrics.pop("f1", 0.0)
            ...
            for k, v in reduced_metrics.items():
                ...
                train_metrics[f"train/{k}"] = v / denom
